...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterFormData) => {
    try {
      ...
      setError(errorMessages[err.code] || 'Erreur lors de l\'inscription');
    }
  };
      ...
      >
        <Card className="border-none shadow-2xl">
          <CardHeader className="text-center space-y-2">
            ...
            <CardDescription>
              {t.auth?.registerDescription || 'Rejoignez la marketplace GuineeGo'}
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="space-y-4">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                {password && (
                  <div className="space-y-1.5 text-xs">
                    <div className={`flex items-center gap-1.5 ${passwordStrength.length ? 'text-guinea-green' : 'text-muted-foreground'}`}>
                      <div className={`w-1.5 h-1.5 rounded-full ${passwordStrength.length ? 'bg-guinea-green' : 'bg-muted'}`} />
                      Minimum 8 caractères
                    </div>
                    <div className={`flex items-center gap-1.5 ${passwordStrength.uppercase ? 'text-guinea-green' : 'text-muted-foreground'}`}>
                      <div className={`w-1.5 h-1.5 rounded-full ${passwordStrength.uppercase ? 'bg-guinea-green' : 'bg-muted'}`} />
                      Au moins une majuscule
                    </div>
                    <div className={`flex items-center gap-1.5 ${passwordStrength.number ? 'text-guinea-green' : 'text-muted-foreground'}`}>
                      <div className={`w-1.5 h-1.5 rounded-full ${passwordStrength.number ? 'bg-guinea-green' : 'bg-muted'}`} />
                      Au moins un chiffre
                    </div>
                  </div>
                )}
              ...
              <Button 
                type="submit" 
                className="w-full bg-guinea-green hover:bg-guinea-green/90"
