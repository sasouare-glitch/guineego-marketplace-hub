...
async function notifySellers(orderId: string, order: any, status: 'delivered' | 'cancelled'): Promise<void> {
  const sellerMessages = {
    delivered: {
      ...
      sms: (amount: string) => `Sarematy: Commande ${orderId} livrée! Montant: ${amount} GNF ajouté à votre solde.`,
    ...
    cancelled: {
      ...
      sms: (_: string) => `Sarematy: Commande ${orderId} annulée. Consultez votre tableau de bord.`,
