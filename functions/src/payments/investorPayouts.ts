...
interface ProcessInvestorReturnsData {
  investmentId?: string;
  investorId?: string;
 ...
 */
export const processInvestorReturns = functions
  .region('europe-west1')
  .https.onCall(async (data: ProcessInvestorReturnsData, context) => {
    verifyAdmin(context);
    ...
    try {
      // Build query
      let query: admin.firestore.Query = db.collection('investments')
        .where('status', '==', 'active');
      ...
      throw new functions.https.HttpsError(
        'internal',
        'Erreur lors du traitement des rendements'
      );
    }
  });
 ...
 */
async function processSingleInvestment(
  investmentDoc: admin.firestore.DocumentSnapshot,
  period: string
): Promise<{
  investorId: string;
  returnAmount: number;
 ...
 */
export const scheduledInvestorReturns = functions
  .region('europe-west1')
 ...
 */
export const processInvestmentMaturity = functions
  .region('europe-west1')
  .https.onCall(async (data: { investmentId: string }, context) => {
    verifyAdmin(context);
    ...
    try {
      return await db.runTransaction(async (transaction) => {
        ...
        if (investment.status !== 'active') {
          throw new functions.https.HttpsError(
            'failed-precondition',
            'L\'investissement n\'est pas actif'
          );
        }
        ...
        if (maturityDate > new Date()) {
          throw new functions.https.HttpsError(
            'failed-precondition',
            `L'investissement n'est pas encore à maturité (${maturityDate.toISOString().slice(0, 10)})`
          );
        }
      ...
      throw new functions.https.HttpsError(
        'internal',
        'Erreur lors du remboursement du capital'
      );
    }
  });
