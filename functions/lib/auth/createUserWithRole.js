 ...
 */
exports.createUserWithRole = functions
    .region('europe-west1')
    ...
    // Validate role
    const validRoles = ['customer', 'ecommerce', 'courier', 'closer', 'investor'];
    ...
    if (data.role === 'admin') {
        if (!context.auth?.token?.role || context.auth.token.role !== 'admin') {
            throw new functions.https.HttpsError('permission-denied', 'Seul un admin peut créer un autre admin');
        }
    }
    try {
        ...
        await batch.commit();
        // 6. Set custom claims
        const claims = { role: data.role, verified: false };
        if (roleDocId) {
            claims[`${data.role}Id`] = roleDocId;
        }
        await (0, auth_1.setUserClaims)(uid, claims);
        return {
            success: true,
            uid,
            role: data.role,
...
//# sourceMappingURL=createUserWithRole.js.map