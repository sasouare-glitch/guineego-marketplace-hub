...
exports.createStripeCheckout = functions
    ...
    if (!orderId || !paymentId) {
        throw new functions.https.HttpsError('invalid-argument', 'orderId et paymentId sont requis');
    }
    try {
        // Get payment record
        ...
        if (!stripeKey) {
            throw new functions.https.HttpsError('failed-precondition', 'Stripe n\'est pas configuré. Configurez la clé via: firebase functions:config:set stripe.secret_key="sk_..."');
        }
        ...
        throw new functions.https.HttpsError('internal', 'Erreur lors de la création de la session Stripe');
    }
});
