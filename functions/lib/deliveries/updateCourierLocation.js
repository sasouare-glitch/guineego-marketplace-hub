...
exports.updateCourierLocation = void 0;
 ...
 */
exports.updateCourierLocation = functions
    .region('europe-west1')
    .https.onCall(async (data, context) => {
    const claims = (0, auth_1.verifyCourier)(context);
    const uid = context.auth.uid;
    const { missionId, latitude, longitude, accuracy, heading, speed } = data;
    if (latitude === undefined || longitude === undefined) {
        throw new functions.https.HttpsError('invalid-argument', 'latitude et longitude sont requis');
    }
    try {
        const location = {
            lat: latitude,
        ...
        if (missionId) {
            ...
            if (missionDoc.exists && missionDoc.data()?.assignedCourier === uid) {
                ...
                if (['picked_up', 'in_transit'].includes(status)) {
                    await checkProximityAndNotify(missionId, missionData, latitude, longitude);
                }
            }
        }
        return { success: true };
        ...
        throw new functions.https.HttpsError('internal', 'Erreur lors de la mise à jour de la position');
    }
});
/**
 ...
 */
async function checkProximityAndNotify(missionId, missionData, courierLat, courierLng) {
    // Skip if already notified
    if (missionData.proximityNotified)
        return;
    ...
    const distance = haversineDistance(courierLat, courierLng, deliveryCoords.lat, deliveryCoords.lng);
    if (distance <= PROXIMITY_THRESHOLD_METERS) {
        await sendProximityNotification(missionId, missionData, Math.round(distance));
    }
}
 ...
 */
async function sendProximityNotification(missionId, missionData, distanceMeters) {
...
async function geocodeAddress(address, commune) {
    ...
    try {
        ...
        const res = await fetch(`https://nominatim.openstreetmap.org/search?${params}`, { headers: { 'User-Agent': 'GuineeGo-Server/1.0' } });
...
//# sourceMappingURL=updateCourierLocation.js.map